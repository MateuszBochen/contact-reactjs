  ...
  render() {
    return (
      <div className="static-modal">
        <Modal.Dialog>
          <Modal.Header>
            <Modal.Title>Tytył wiadomośći kontaktowej</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <FormGroup>
              <InputGroup>
            ...
            <h5>Gotowe tematy do wybrania z listy:</h5>
            <Form horizontal>
              <NewDropdownOption addNew={this.onAddNewOption} title="Temat:" />

              { this.state.options.map((subject, index) =>
                (<NewDropdownOption value={subject} key={`${subject}+${index}`} index={index} delete={this.onDeleteOption} title="Temat:" update={this.onUpdateOption} />)
              )}
            </Form>
          </Modal.Body>
          <Modal.Footer>
